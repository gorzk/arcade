...
class Hud():
	def __init__(self, single_sprites, menu, sounds):
		...
		self.textline1 = "W kosmosie odnotowano niepokojacy"
		self.textline2 = "wzrost poziomu lewactwa. Tylko ty"
		self.textline3 = "jestes w stanie powstrzymac ta plage"
		self.textline4 = "i raz na zawsze unicestwic zlo."
	def blit(self, ship, level):
		...
		if level==0:
			if self.johnsonOffset%1==0 and self.johnsonOffset>30 and self.johnsonOffset<240:
			...
			if self.johnsonOffset==230:
				...
				if self.johnsonOffset<30:
					if self.johnsonOffset%8>4:
						screen.blit(self.communicationCall.render("Polaczenie przychodzace", True, (255,255,255)),(20, 15))
				elif self.johnsonOffset<=240:
					...
					if self.johnsonOffset == 241:
						self.menu.play_sound(self.sounds["noise.wav"])
					if self.johnsonOffset%8>4:
						screen.blit(self.communicationCall.render("Koniec transmisji", True, (255,255,255)),(20, 15))				
		return level
